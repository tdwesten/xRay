...
const Sidebar = () => {
  const { projects, selectedProject, showBookmarksOnly, updateFilters } =
    useLog();
  ...
  const renderSectionHeader = (title) => (
    <div className="flex items-center w-full px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-200">
      <span className="text-xs tracking-widest uppercase">{title}</span>
    </div>
  );

  return (
    <div className="flex flex-col w-64 bg-white dark:bg-gray-800 dark:border-gray-700">
      <div className="flex items-center px-4 h-14 bg-gray-50 dark:bg-gray-800/80">
        <div className="flex items-center space-x-2">
          <div
            className={`w-4 h-4 ${
