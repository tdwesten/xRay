...
const FilterBar = () => {
  const { clearLogs, selectedLevels, updateFilters, totalLogsCount } = useLog();

  const toggleLogLevel = (logLevel) => {
    if (selectedLevels.has(logLevel)) {
      updateFilters({
        levels: new Set(
          [...selectedLevels].filter((level) => level !== logLevel)
        ),
      });
    } else {
      updateFilters({
  ...
  return (
    <div className="z-10 bg-white dark:bg-gray-800">
        ...
        <div className="flex items-center space-x-1">
          {LOG_LEVELS.map(({ level, color }) => (
            <button
              ...
              className="flex items-center px-2 py-1 space-x-1 text-xs capitalize transition-colors rounded-md"
              style={{
                ...
                border: `1px solid ${
                  selectedLevels.has(level) ? color : "transparent"
                }`,
              }}
            >
              <div
